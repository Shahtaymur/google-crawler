...
NB_REQUESTS_LIMIT = os.environ.get(
    "RELATED_QUESTION_NBREQUESTS_LIMIT", 10
)
...
def get_related_answer(question: str, depth: bool = False) -> str:
    """
    return a text as summary answer for the question
    ...
    if featured_snippet:
        data = {
            "date":featured_snippet.date,
            "displayed_link":featured_snippet.displayed_link,
            "heading":featured_snippet.heading,
            "link":featured_snippet.link,
            "raw_text":featured_snippet.raw_text,
            "response":featured_snippet.raw_text,
            "snippet_data":featured_snippet.snippet_data,
            "snippet_str":featured_snippet.snippet_str,
            "snippet_str_body":featured_snippet.snippet_str_body,
            "snippet_type":featured_snippet.snippet_type,
            "text":featured_snippet.text,
            "title":featured_snippet.title,
            "question":question
            }
        return data
    if depth:
        ...
        if not related_questions:
            return ""
        return get_related_answer(related_questions[0])
    return ""
